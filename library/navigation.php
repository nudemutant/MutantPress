 ...
 * Multi Level Menu
 */
if ( ! function_exists( 'multi_level_menu' ) ) {
    function multi_level_menu() {
        wp_nav_menu(array(
            'container' => false,                           // Remove nav container
            'container_class' => '',                        // Class of container
            'menu' => '',                                   // Menu name
            'menu_class' => 'menu__wrap',                   // Adding custom nav class
            'theme_location' => '',                         // Where it's located in the theme
            'before' => '',                                 // Before each link <a>
            'after' => '',                                  // After each link </a>
            'link_before' => '',                            // Before each link text
            'link_after' => '',                             // After each link text
            'items_wrap' => '<div id="%1$s" class="%2$s">%3$s</div>',// Replacing the ul with div
            'depth' => 0,                                   // Limit the depth of the nav
            'fallback_cb' => false,                         // Fallback function (see below)
            'walker' => new Mutantpress_Multi_Level_Menu_Walker(),
        ));
    }
}
